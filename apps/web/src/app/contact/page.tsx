import type { Metadata } from 'next';
import Link from 'next/link';

export const metadata: Metadata = {
  title: 'Connect',
  description:
    'Connect with Milos Cvetkovic on LinkedIn, GitHub, and X. Follow along for engineering insights and project updates.',
  openGraph: {
    title: 'Connect | Milos Cvetkovic',
    description: 'Follow along for engineering insights, open-source work, and project updates.',
  },
};

const socialLinks = [
  {
    name: 'LinkedIn',
    href: 'https://www.linkedin.com/in/milos-cvetkovic-dev',
    description: 'Engineering insights, career updates, and professional connections.',
    icon: (
      <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
    cta: 'Connect on LinkedIn',
    primary: true,
  },
  {
    name: 'GitHub',
    href: 'https://github.com/milosCvetkovicDev',
    description: 'Open-source projects, code contributions, and technical explorations.',
    icon: (
      <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
      </svg>
    ),
    cta: 'Follow on GitHub',
    primary: false,
  },
  {
    name: 'X / Twitter',
    href: 'https://x.com/milos_dev',
    description: 'Quick takes on engineering, AI, and tech trends.',
    icon: (
      <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
    cta: 'Follow on X',
    primary: false,
  },
];

export default function ContactPage() {
  return (
    <div className="py-16 md:py-24">
      <div className="mx-auto max-w-3xl px-6">
        {/* Header */}
        <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">
          Let&apos;s connect.
        </h1>
        <p className="text-xl text-[var(--muted)] mb-12">
          I share what I&apos;m building, lessons from production, and engineering
          insights. Pick your preferred platform and say hi.
        </p>

        {/* Social links */}
        <section className="mb-16">
          <div className="space-y-4">
            {socialLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-6 p-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] hover:border-[var(--accent)]/50 transition-colors group"
              >
                <div className="text-[var(--muted)] group-hover:text-[var(--accent)] transition-colors">
                  {link.icon}
                </div>
                <div className="flex-1">
                  <h2 className="font-semibold text-lg mb-1">{link.name}</h2>
                  <p className="text-sm text-[var(--muted)]">{link.description}</p>
                </div>
                <span className={`hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-lg font-medium text-sm transition-colors ${
                  link.primary
                    ? 'bg-[var(--accent)] text-white group-hover:bg-[var(--accent-hover)]'
                    : 'border border-[var(--border)] group-hover:border-[var(--accent)]/50'
                }`}>
                  {link.cta}
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                  </svg>
                </span>
              </a>
            ))}
          </div>
        </section>

        {/* Browse work */}
        <section className="text-center p-8 rounded-2xl border border-[var(--border)] bg-[var(--card)]">
          <p className="text-lg mb-4">
            Want to see what I&apos;ve been working on?
          </p>
          <p className="text-[var(--muted)] mb-6">
            Check out my latest projects, case studies, and the engineering behind them.
          </p>
          <Link
            href="/work"
            className="inline-flex items-center gap-2 px-6 py-3 border border-[var(--accent)] text-[var(--accent)] font-semibold rounded-lg hover:bg-[var(--accent)]/10 transition-colors"
          >
            <span>View My Work</span>
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
            </svg>
          </Link>
        </section>
      </div>
    </div>
  );
}
