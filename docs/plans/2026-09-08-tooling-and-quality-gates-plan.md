...
Why `fileURLToPath(import.meta.resolve(...))`: Prettier resolves plugin *names* relative to the config file that lists them. With pnpm's strict `node_modules`, `apps/web` cannot see a plugin that only `packages/prettier-config` depends on. An absolute path sidesteps that.
...
- [ ] **Step 1:** change the React import to `import { useCallback, useEffect, useRef, useState } from 'react';`, move the whole `const animateHealing = () => { ... };` block *above* the `useEffect`, wrap it as
