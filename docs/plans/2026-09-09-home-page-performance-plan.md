...
| D4  | Story sections are code-split behind their own Suspense boundary and their markup stays in the document. Deferring their hydration was shipped and then reverted: React discards server markup when a boundary suspends during hydration. | Mounting all six at load was the largest task in the trace, but a section that leaves the DOM after hydration is out of find-in-page and out of screen-reader browse mode, which is a worse defect than the one being fixed. Deferring the GSAP work instead of the hydration is the open follow-up. |
