...
| D4  | Story sections below the hero mount when their placeholder is about to enter the viewport (IntersectionObserver), and their chunks are prefetched during idle time so mounting is instant. | Mounting all six at load was the largest task in the trace and none of them is visible at load. Prefetching keeps the scroll experience unchanged on the connections the site targets.                                              |
...
- [x] `DeferredSection` wraps each lazy phase: renders the `min-h-screen` placeholder until an `IntersectionObserver` reports it near the viewport, then the phase (still through `Suspense` and `React.lazy`). No `IntersectionObserver` means mount after hydration.
- [x] Prefetch the six phase modules once, on the first pointer, touch, key or scroll event or after 3 s idle (`requestIdleCallback` with a timeout fallback). Idle-only prefetch was measured first and rejected: it evaluated the shared GSAP chunk (33 ms observed, 132 ms at 4×) at 0.7 s, inside the window Lighthouse scores.
...
- [x] e2e: observe `layout-shift` entries for four seconds of log activity and assert the summed value stays under 0.005.
