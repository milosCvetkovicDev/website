...
| D4  | The story sections are imported directly and render with the page. Deferring their hydration shipped and was reverted (React discards server markup when a boundary suspends during hydration); leaving them lazy without that gate was measured and rejected too (the placeholder-to-content swap costs CLS 0.092 against 0.016). | Mounting all six at load was the largest task in the trace, but a section that leaves the DOM after hydration is out of find-in-page and out of screen-reader browse mode, and a section that swaps in late shifts the page. Deferring the GSAP work instead of the content is the open follow-up. |
