...
- [ ] MADR-style sections in each: Status, Date, Context, Decision, Consequences, Alternatives considered. Index in `docs/adr/README.md`. Commit: `docs(adr): record toolchain, quality, hosting and client-state decisions`.
...
- [ ] Index table: document, type (design / plan), status (Shipped, In progress, Planned) with PR links. Commit: `docs(plans): index plans and mark the February work as shipped`.

### Task C6: Verify, push, PR

- [ ] Link check: `node -e` script that extracts relative markdown links from the new files and asserts each target exists.
- [ ] `pnpm format:check`; push; `gh pr create --base chore/tooling-and-quality-gates`; watch CI.
