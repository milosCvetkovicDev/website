...
dark-theme audit; the light theme is checked with Playwright and axe-core instead. The same rule
set now runs in CI on every pull request: `apps/web/e2e/accessibility.spec.ts` audits both pages
in both colour schemes at the desktop viewport, at rest, and fails the `e2e` job on any violation.
The home page
