...
| Node 22           | Matches `.nvmrc` (`22`) and `engines.node` (`^22.22.2 \|\| ^24.15.0 \|\| >=26.0.0`) in the root `package.json`. `nvm use`, and `nvm install 22` if pnpm prints `WARN Unsupported engine`.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| pnpm 10.34.5      | Pinned by `packageManager` in the root `package.json`. Use corepack rather than a global pnpm.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
Both jobs, and the `Commit messages` job in `.github/workflows/commitlint.yml`, must be green before a pull request can merge ([ADR 0021](../adr/0021-squash-only-merges-and-required-checks.md)), and merging to `main` is what deploys. Vercel runs none of
...
- **Node version.** `apps/web/package.json` has no `engines` field, and Vercel reads the Node version
  ...
  (`^22.22.2 || ^24.15.0 || >=26.0.0`) and `.nvmrc` (`22`) never reach it. Locally, `nvm use` does
  read `.nvmrc`.
