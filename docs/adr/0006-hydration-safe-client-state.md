...
name and the file. No `themeInitScript` exists in the tree this record was accepted in, and the
script string was never defined in `layout.tsx`. The rest of that paragraph is not corrected.
...
13; and `git grep -n themeInitScript a8b4a91 -- apps` prints nothing. Found by
