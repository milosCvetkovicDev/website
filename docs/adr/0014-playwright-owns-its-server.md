...
wrong: the number, not the line it describes. This entry withdraws the citation
`apps/web/playwright.config.ts:64` in both places the 2026-09-12 entry gives it; the pin is on
line 104. The pin had sat at `:64` since 5a02529, and the entry was written against d49b36d, the
pull request's head before it was rebuilt on 0f7c673, where it still was. 0f7c673 (#68) added 41
lines above it and merged thirteen minutes before 4c02b40. The entry's reading of what that line
does is untouched.
...
`git show 5a02529:apps/web/playwright.config.ts | sed -n 64p` prints the `NEXT_DIST_DIR` line;
`git log -1 --format=%B 4c02b40` closes with "Previous head: d49b36d.", and
`git show d49b36d:apps/web/playwright.config.ts | sed -n 64p` prints the `NEXT_DIST_DIR` line too;
and
