 ...
 * a plain string comparison is false whenever the invocation path crosses a symlink. In a CI gate
 * that is a check that silently does not run; here it is worse, because a script that never calls
 * `main()` exits 0 and Vercel reads 0 as "skip" — production included.
 *
